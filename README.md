...
| 層 | 検査するもの |
|---|---|
| 単体 `tests/` | 盤の規則、最短手数の算出、出題の保証、保存値の検証、配色の不変条件、集計、配るファイルの一覧と実物の食い違い |
| E2E `e2e/` | 遊びの流れ（押す・戻す・使い切り・校了・自動遷移）、表示設定と保存、キーボード操作、壊れた保存値、保存不可環境、CSP、モジュールの読み込み、狭い画面、圏外での起動と操作 |
